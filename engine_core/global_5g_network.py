...
class ContainerGlobal5G:
    ...
    def integrate_5g(self, network: Global5GNetwork):
        ...
        closest_node, distance = network.find_optimal_node(
...
def main():
    ...
    for container_name, container_id, lat, lon in containers:
        ...
        five_g_info = container.integrate_5g(network)

        print(f"\n{container_name}:")
        print(f"  Location: ({lat:.4f}, {lon:.4f})")
        print(f"  Closest 5G Node: {five_g_info['closest_5g_node']}")
        print(f"  Distance: {five_g_info['distance_km']:.2f} km")
        print(f"  Signal: {five_g_info['signal_strength_dbm']} dBm")
        print(f"  Latency: {five_g_info['latency_ms']} ms")
        print(f"  Band: {five_g_info['band']}")
    ...
    container = ContainerGlobal5G(
        "ultimate-engine", "CONTAINER_ULTIMATE", 51.5074, -0.1278
    )
